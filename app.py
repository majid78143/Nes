...
# ── Firebase init — reads from FIREBASE_CREDENTIALS_JSON env variable ─────────
import json as _json
db = None
_firebase_json = os.environ.get("FIREBASE_CREDENTIALS_JSON", "").strip()
# Remove accidental surrounding quotes if user pasted with them
if _firebase_json.startswith('"') and _firebase_json.endswith('"'):
    _firebase_json = _firebase_json[1:-1].replace('\\"', '"')
if _firebase_json:
    try:
        _sa = _json.loads(_firebase_json)
        _cred = credentials.Certificate(_sa)
    ...
    print("WARNING: FIREBASE_CREDENTIALS_JSON env variable not set. Database disabled.")

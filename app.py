import os, uuid, re, secrets, requests, smtplib
...
# ── Config ────────────────────────────────────────────────────────────────────
# BUG FIX #5: Pehle har restart pe random key banti thi — sab sessions toot jaate the.
# Ab agar SECRET_KEY env var nahi hai toh warning aur ek fixed fallback use karo.
_secret = os.environ.get("SECRET_KEY")
if not _secret:
    print("WARNING: SECRET_KEY env var set nahi hai! Admin sessions server restart pe toot jaayenge. Set karo: export SECRET_KEY=<koi_bhi_lamba_random_string>")
    _secret = "tezkhabar-default-key-change-this-in-production-please"
app.secret_key = _secret
app.config["PERMANENT_SESSION_LIFETIME"] = timedelta(days=1)

SITE_NAME    = "TezKhabar"
SITE_TAGLINE = "Breaking News · 24/7"
SITE_URL     = os.environ.get("SITE_URL", "https://tezkhabar.online")
...
# BUG FIX #1 & #2: AD_SLOT_HEADER pehle pura HTML tha (wrong!) — ab sirf slot ID hai.
# base.html ke <head> mein already AdSense script load hota hai, yahan sirf ID chahiye.
# BUG FIX #3: AD_SLOT_ARTICLE aur AD_SLOT_FOOTER ke fake IDs (3333333333, 4444444444)
# real IDs se replace karo — yahan sidebar ID use ki hai temporarily.
# ⚠ IMPORTANT: Apne Google AdSense account se real slot IDs bharo yahan.
AD_SLOT_HEADER       = "5369764793"   # Homepage top ad slot ID
AD_SLOT_SIDEBAR      = "5369764793"   # Sidebar ad slot ID
AD_SLOT_ARTICLE      = "5369764793"   # Article in-article ad slot ID — apna real ID dalo
AD_SLOT_FOOTER       = "5369764793"   # Footer ad slot ID — apna real ID dalo
...
def get_settings():
    defaults = {"breaking_news":"TezKhabar pe aapka swagat hai!","maintenance_mode":False,"announcement":"","user_comments_enabled":False}
    if not db: return defaults.copy()
    try:
...
def validate_csrf():
    """POST requests pe CSRF token verify karo"""
...
def inject_globals():
    # Login page sirf CSRF chahta hai — Firebase/settings load karne ki zaroorat nahi.
    ...
    try:
        ...
        app.logger.exception("inject_globals failed")
        # Safe fallback — page crash nahi hoga agar settings load na ho
        defaults={"breaking_news":"TezKhabar pe aapka swagat hai!","maintenance_mode":False,"announcement":"","user_comments_enabled":False}
        return dict(site_name=SITE_NAME,site_tagline=SITE_TAGLINE,logo_url=LOGO_URL,
                    adsense_pub=ADSENSE_PUB,adsense_enabled=ADSENSE_ENABLED,
                    ad_slot_header=AD_SLOT_HEADER,ad_slot_sidebar=AD_SLOT_SIDEBAR,
                    ad_slot_in_article=AD_SLOT_ARTICLE,ad_slot_footer=AD_SLOT_FOOTER,
                    google_verification=GOOGLE_VERIFICATION,settings=defaults,
                    now=datetime.utcnow(),time_ago=time_ago,all_categories=CATEGORIES,
                    site_url=request.url_root.rstrip("/"),
                    is_superadmin=(session.get("admin_role")=="superadmin"),
                    is_admin=(session.get("admin_role") in ["admin","superadmin"]),
...
def index():
    ...
    if db:
        try:
            ...
            print(f"Articles fetch error: {e}")
...
def article(slug):
    ...
    related=[]
    try:
        # BUG FIX #7: Pehle related articles ka koi order nahi tha — purane random articles aate the.
        # Ab published_at ke order mein latest related articles aate hain.
        related=[{"id":r.id,**r.to_dict()} for r in db.collection("articles")
                 .where("category","==",art.get("category",""))
                 .where("status","==","published")
    ...
    author_ach=compute_achievements(art.get("author_id","")) if art.get("author_id") else []
    return render_template("article.html",article=art,related=related,comments=comments,author_achievements=author_ach,all_achievements=ACHIEVEMENTS)
...
def category(name):
    ...
    if db:
        try:
            # FIX: Pehle 3 conditions + order_by tha — composite Firestore index chahiye tha
            # jो nahi bana tha, isliye silently empty list aati thi.
            # Ab sirf ek where("category") — baaki Python mein filter + sort.
            raw=[{"id":d.id,**d.to_dict()} for d in
                 db.collection("articles").where("category","==",name).limit(300).stream()]
            arts=[a for a in raw if a.get("status")=="published"]
            arts.sort(key=lambda x: x.get("published_at") or datetime.min, reverse=True)
        except Exception as e:
...
def tag_page(tag):
    ...
    if db:
        try:
            # BUG FIX #8 (cont): Pehle poori collection scan hoti thi bina limit ke.
            # Ab sirf latest 300 articles mein tag dhundho, aur max 50 results.
            for d in db.collection("articles").where("status","==","published").order_by("published_at",direction=firestore.Query.DESCENDING).limit(300).stream():
                a={"id":d.id,**d.to_dict()}
...
def author_page(author_id):
    ...
    achievements=compute_achievements(author_id)
    return render_template("author.html",author={"id":doc.id,**doc.to_dict()},articles=arts,achievements=achievements,all_achievements=ACHIEVEMENTS)
...
def search():
    ...
    if q and db:
        ...
        try:
            ql=q.lower()
            # BUG FIX #8: Pehle poore DB ka scan hota tha — ab sirf latest 500 articles mein search
            # aur turant 30 results pe rok diya. Bada DB slow nahi hoga.
            for d in db.collection("articles").where("status","==","published").order_by("published_at",direction=firestore.Query.DESCENDING).limit(500).stream():
                a={"id":d.id,**d.to_dict()}
                if ql in a.get("title","").lower() or ql in a.get("summary","").lower():
                    if not cat or a.get("category")==cat:
                        results.append(a)
                        if len(results)>=30: break  # zyada results nahi chahiye
        except Exception as e:
...
def react(article_id):
    # FIX: request.json ke jagah get_json(silent=True) — crash nahi hoga agar JSON na ho
...
def admin_login():
    try:
        ...
        app.logger.exception("Admin login page failed")
...
def send_reset_email(to_email, reset_link):
    """Gmail SMTP se password reset email bhejo."""
    gmail_user = os.environ.get("GMAIL_USER","")
    gmail_pass = os.environ.get("GMAIL_APP_PASSWORD","")
    if not gmail_user or not gmail_pass:
        app.logger.error("GMAIL_USER ya GMAIL_APP_PASSWORD set nahi hai.")
        return False
    try:
        msg = MIMEMultipart("alternative")
        msg["Subject"] = "TezKhabar Admin — Password Reset Link"
        msg["From"]    = f"TezKhabar Admin <{gmail_user}>"
        msg["To"]      = to_email
        html_body = f"""
        <div style="font-family:sans-serif;max-width:480px;margin:auto;background:#111;border:1px solid #222;border-radius:12px;padding:32px;color:#ddd">
          <img src="{LOGO_URL}" alt="TezKhabar" style="height:40px;margin-bottom:24px;display:block">
          <h2 style="color:#fff;margin-bottom:8px">Password Reset</h2>
          <p style="color:#888;margin-bottom:24px;font-size:14px">Aapne password reset request ki hai. Neeche ka button dabao:</p>
          <a href="{reset_link}" style="display:inline-block;background:#cc0000;color:#fff;padding:12px 24px;border-radius:8px;text-decoration:none;font-weight:700;font-size:15px">Reset Password →</a>
          <p style="color:#444;font-size:12px;margin-top:24px">Yeh link <strong>1 ghante</strong> mein expire ho jaayega.<br>Agar aapne request nahi ki toh ignore karo.</p>
          <hr style="border-color:#222;margin-top:24px">
          <p style="color:#333;font-size:11px">TezKhabar Admin Panel</p>
        </div>"""
        ...
        app.logger.exception("Reset email bhejne mein error")
        return False
...
def admin_forgot_password():
    ...
    if request.method == "POST":
        ...
        if not email:
            ...
            # Super Admin ke liye reset allow nahi — password ENV se hai
            if email == SUPER_ADMIN_EMAIL.lower():
                error = "Super Admin ka password Render ENV variables se change hota hai."
            else:
                found = False
                if db:
                    try:
                        ...
                        if docs:
                            ...
                            if sent:
                                msg = "Reset link aapki email pe bhej diya gaya hai। 1 ghante mein expire hoga।"
                            else:
                                error = "Email nahi ja saki। Server pe GMAIL_USER aur GMAIL_APP_PASSWORD set karo।"
                    except Exception:
                        app.logger.exception("Forgot password DB error")
                        error = "Kuch gadbad ho gayi। Dobara try karo।"
                if not found and not error and not msg:
                    # Security: email exist kare ya na kare, same message
                    msg = "Agar yeh email registered hai to reset link bhej diya gaya है।"
...
def admin_reset_password(token):
    ...
    error = None
    if not db:
        return render_template("reset_password.html", error="Database unavailable।", token=token, valid=False)
    try:
        doc = db.collection("password_resets").document(token).get()
    except Exception:
        return render_template("reset_password.html", error="Invalid link।", token=token, valid=False)
    if not doc.exists:
        return render_template("reset_password.html", error="Link invalid ya expire ho gayi।", token=token, valid=False)
    data = doc.to_dict()
    if data.get("used"):
        return render_template("reset_password.html", error="Yeh link pehle se use ho chuki hai।", token=token, valid=False)
    expires = data.get("expires_at")
    try:
        if expires and datetime.utcnow() > expires.replace(tzinfo=None):
            return render_template("reset_password.html", error="Link expire ho gayi। Nayi request karo।", token=token, valid=False)
    except Exception:
        pass
    if request.method == "POST":
        ...
        if len(new_pw) < 8:
            ...
            try:
                db.collection("users").document(data["user_id"]).update({"password": hash_pw(new_pw)})
                db.collection("password_resets").document(token).update({"used": True})
                flash("Password successfully change ho gaya! Ab login karo।","success")
                return redirect(url_for("admin_login"))
            except Exception:
                app.logger.exception("Reset password update error")
                error = "Password update nahi hua। Dobara try karo।"
    return render_template("reset_password.html", error=error, token=token, valid=True, email=data.get("email",""))
...
def admin_dashboard():
    if not db:
        return render_template("admin_dashboard.html",articles_count=0,published=0,users_count=0,total_views=0,recent=[])
    try:
        ...
        all_arts=[{"id":d.id,**d.to_dict()} for d in arts]
        total_views=sum(a.get("views",0) for a in all_arts if a.get("status")=="published")
        users_count=len(list(db.collection("users").stream()))
        return render_template("admin_dashboard.html",articles_count=len(arts),published=pub,
                               users_count=users_count,total_views=total_views,recent=recent)
    except Exception:
        app.logger.exception("admin_dashboard Firebase query failed")
        return render_template("admin_dashboard.html",articles_count=0,published=0,users_count=0,total_views=0,recent=[])
...
def admin_upload_image():
    # BUG FIX #6: File type aur size check add kiya — pehle koi bhi file upload ho sakti thi
    ...
    if ext not in allowed_ext:
        return jsonify({"error":f"Sirf images allowed hain ({', '.join(allowed_ext)})"}),400
    data = f.read()
    if len(data) > 5 * 1024 * 1024:  # 5MB limit
        return jsonify({"error":"File 5MB se badi hai"}),400
    url=upload_image(data, f.filename)
...
@login_required
def admin_profile():
    is_superadmin_user = (session.get("admin_role") == "superadmin")
    # Super Admin ka profile Firestore mein "superadmin" document mein store hota hai
    doc_id = "superadmin" if is_superadmin_user else session["admin_id"]
    if not db:
        user = {"id": doc_id, "name": session.get("admin_name","Super Admin"),
                "email": session.get("admin_email", SUPER_ADMIN_EMAIL),
                "role": session.get("admin_role","superadmin"),
                "bio":"", "designation":"", "profile_pic":""}
    else:
        doc = db.collection("users").document(doc_id).get()
        if doc.exists:
            user = {"id": doc.id, **doc.to_dict()}
        else:
            # Pehli baar — default values
            user = {"id": doc_id, "name": session.get("admin_name","Super Admin"),
                    "email": session.get("admin_email", SUPER_ADMIN_EMAIL),
                    "role": session.get("admin_role","superadmin"),
                    "bio":"", "designation":"", "profile_pic":""}
    if request.method == "POST":
        ...
        if "profile_pic" in request.files:
            ...
            if f and f.filename:
                nu = upload_image(f.read(), f.filename)
                pic_url = nu if nu else pic_url
        upd = {
            "name": request.form.get("name","").strip() or user.get("name",""),
        ...
        # Password change — sirf non-superadmin ke liye
        if not is_superadmin_user:
            old = request.form.get("old_password","")
            new = request.form.get("new_password","")
            if old and new:
                if user.get("password") == hash_pw(old):
                    upd["password"] = hash_pw(new)
                    flash("Password update ho gaya.", "success")
                else:
                    flash("Purana password galat hai.", "danger")
        if db:
            db.collection("users").document(doc_id).set(upd, merge=True)
        session["admin_name"] = upd["name"]
        flash("Profile update ho gayi!", "success")
        return redirect(url_for("admin_profile"))
    achievements = compute_achievements(doc_id)
    return render_template("admin_profile.html", user=user, achievements=achievements,
                           all_achievements=ACHIEVEMENTS, is_superadmin_user=is_superadmin_user)

@app.route("/admin/analytics")
@login_required
def admin_analytics():
    if not db: return render_template("admin_analytics.html",top_articles=[],top_searches=[])
    top=[{"id":d.id,**d.to_dict()} for d in db.collection("articles").where("status","==","published")
         .order_by("views",direction=firestore.Query.DESCENDING).limit(10).stream()]
    searches={}
    try:
...
# ═══════════════════════════════════════════════════════════════════════════════
ACHIEVEMENTS = [
    {"id":"first_quill","name":"Pehli Kadam","desc":"Pehla article publish kiya"},
    {"id":"ten_club",   "name":"Dus Ka Dum", "desc":"10 articles publish kiye"},
    {"id":"century",    "name":"Shatabdi",   "desc":"100 articles publish kiye"},
    {"id":"viral",      "name":"Viral Star", "desc":"Koi article 1000+ views mila"},
    {"id":"lightning",  "name":"Speed Writer","desc":"Ek din mein 3+ articles"},
    {"id":"night_owl",  "name":"Night Owl",  "desc":"Raat 11baje ke baad article publish kiya"},
    {"id":"consistent", "name":"Iron Pen",   "desc":"7 alag din articles publish kiye"},
]

def compute_achievements(user_id):
    if not db: return []
    unlocked = []
    try:
        from collections import Counter
        arts = [a.to_dict() for a in db.collection("articles").where("author_id","==",user_id).where("status","==","published").stream()]
        count = len(arts)
        max_views = max((a.get("views",0) for a in arts), default=0)
        day_counts = Counter()
        has_night = False
        for a in arts:
            pub = a.get("published_at")
            if pub and hasattr(pub,"date"):
                day_counts[pub.date()] += 1
                if pub.hour >= 23 or pub.hour < 5:
                    has_night = True
        daily_max = max(day_counts.values(), default=0)
        unique_days = len(day_counts)
        if count >= 1:   unlocked.append("first_quill")
        if count >= 10:  unlocked.append("ten_club")
        if count >= 100: unlocked.append("century")
        if max_views >= 1000: unlocked.append("viral")
        if daily_max >= 3:   unlocked.append("lightning")
        if has_night:        unlocked.append("night_owl")
        if unique_days >= 7: unlocked.append("consistent")
    except: pass
    return unlocked

@app.route("/api/achievements/<user_id>")
def api_achievements(user_id):
    unlocked = compute_achievements(user_id)
    return jsonify({"achievements": unlocked, "all": ACHIEVEMENTS})
...
def admin_chat_list():
    ...
    if db:
        try:
            # FIX: Pehle where("active") + order_by("created_at") tha — composite index
            # nahi tha isliye silently empty list aati thi, rooms nahi dikhte the.
            # Ab saare rooms fetch karo, Python mein active filter + sort karo.
            all_rooms = [{"id":d.id,**d.to_dict()} for d in db.collection("chat_rooms").stream()]
            rooms = [r for r in all_rooms if r.get("active", False)]
            rooms.sort(key=lambda x: x.get("created_at") or datetime.min, reverse=True)
        except Exception as e:
            print(f"Chat rooms fetch error: {e}")
    return render_template("admin_chat_list.html", rooms=rooms)

@app.route("/admin/chat/create", methods=["POST"])
@superadmin_required
def admin_chat_create():
    validate_csrf()
    name = request.form.get("name","").strip()
    if name and db:
        db.collection("chat_rooms").add({"name":name,"created_by":session.get("admin_name","Super Admin"),"created_at":datetime.utcnow(),"active":True})
        flash(f"Room '{name}' create ho gayi!","success")
    return redirect(url_for("admin_chat_list"))

@app.route("/admin/chat/<room_id>/close", methods=["POST"])
@superadmin_required
def admin_chat_close(room_id):
    validate_csrf()
    if db: db.collection("chat_rooms").document(room_id).update({"active":False})
    flash("Room band kar di.","info"); return redirect(url_for("admin_chat_list"))

@app.route("/admin/chat/<room_id>")
@login_required
def admin_chat_room(room_id):
    if not db: abort(404)
    rdoc = db.collection("chat_rooms").document(room_id).get()
    if not rdoc.exists or not rdoc.to_dict().get("active"): abort(404)
    room = {"id":rdoc.id,**rdoc.to_dict()}
    msgs = []
    try:
        msgs = [{"id":m.id,**{k:(v.isoformat() if hasattr(v,"isoformat") else v) for k,v in m.to_dict().items()}} for m in db.collection("chat_rooms").document(room_id).collection("messages").order_by("timestamp").limit(100).stream()]
    except Exception as e:
        print(f"Chat messages fetch error: {e}")
    members = [{"name":"Super Admin","id":"superadmin"}]
    try:
        for u in db.collection("users").stream(): members.append({"name":u.to_dict().get("name",""),"id":u.id})
    except: pass
    return render_template("chat_room.html", room=room, messages=msgs, members=members)

@app.route("/admin/chat/<room_id>/send", methods=["POST"])
@login_required
def admin_chat_send(room_id):
    if not db: return jsonify({"error":"DB offline"}),500
    data = request.get_json(silent=True) or {}
    text = data.get("text","").strip()
    if not text: return jsonify({"error":"Empty"}),400
    if len(text) > 1000: return jsonify({"error":"Message bahut lamba hai"}),400
    reply_to = data.get("reply_to")
    mentions = re.findall(r"@(\S+)", text)
    db.collection("chat_rooms").document(room_id).collection("messages").add({
        "text":text,"author":session.get("admin_name","Unknown"),
        "author_id":session.get("admin_id",""),"timestamp":datetime.utcnow(),
        "mentions":mentions[:10],"reply_to":reply_to
    })
    return jsonify({"ok":True})

@app.route("/admin/chat/<room_id>/messages")
@login_required
def admin_chat_messages(room_id):
    if not db: return jsonify({"messages":[]})
    try:
        # FIX: Pehle order_by("timestamp") tha — agar Firestore index nahi bana toh fail hota.
        # Ab bina order_by ke fetch karo, Python mein sort karo.
        raw = [{"id":m.id,**{k:(v.isoformat() if hasattr(v,"isoformat") else v) for k,v in m.to_dict().items()}}
               for m in db.collection("chat_rooms").document(room_id).collection("messages").limit(150).stream()]
        msgs = sorted(raw, key=lambda x: x.get("timestamp",""))
    except Exception as e:
...
# ADMIN NOTICES
# ═══════════════════════════════════════════════════════════════════════════════
@app.route("/admin/notice/send", methods=["POST"])
@superadmin_required
def admin_notice_send():
    validate_csrf()
    text = request.form.get("notice_text","").strip()
    if text and db:
        db.collection("admin_notices").add({"text":text,"created_by":session.get("admin_name","Super Admin"),"created_at":datetime.utcnow(),"active":True})
        flash("Notice bhej diya!","success")
    return redirect(url_for("admin_settings"))

@app.route("/admin/notice/<nid>/dismiss", methods=["POST"])
@login_required
def admin_notice_dismiss(nid):
    d = session.get("dismissed_notices",[])
    if nid not in d: d.append(nid)
    session["dismissed_notices"] = d
    return jsonify({"ok":True})

@app.route("/api/admin/notices")
@login_required
def api_admin_notices():
    if not db: return jsonify({"notices":[]})
    try:
        dismissed = session.get("dismissed_notices",[])
        notices = [{"id":d.id,**{k:(v.isoformat() if hasattr(v,"isoformat") else v) for k,v in d.to_dict().items()}} for d in db.collection("admin_notices").where("active","==",True).order_by("created_at",direction=firestore.Query.DESCENDING).limit(3).stream() if d.id not in dismissed]
        return jsonify({"notices":notices})
    except: return jsonify({"notices":[]})

...
# ── Firebase service account — replace values with your real ones ─────────────
FIREBASE_SA = {
    "type": "service_account",
    "project_id": "dreamdrop-3ca3d",
    "private_key_id": "3f2c5a2230e3f5f9ba19e2f3df1123c620a700d9",
    "private_key": "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n",
    "client_email": "firebase-adminsdk-xxxx@dreamdrop-3ca3d.iam.gserviceaccount.com",
    "client_id": "000000000000000000000",
    "auth_uri": "https://accounts.google.com/o/oauth2/auth",
    "token_uri": "https://oauth2.googleapis.com/token",
    "auth_provider_x509_cert_url": "https://www.googleapis.com/oauth2/v1/certs",
    "client_x509_cert_url": "https://www.googleapis.com/robot/v1/metadata/x509/firebase-adminsdk-xxxx%40dreamdrop-3ca3d.iam.gserviceaccount.com"
}

# ── Super Admin credentials from Render ENV only ─────────────────────────────
...
def globals():
    ...
    return dict(site_name=SITE_NAME,site_tagline=SITE_TAGLINE,logo_url=LOGO_URL,
                ...
                site_url=SITE_URL,is_superadmin=(session.get("admin_role")=="superadmin"),
...
def article(slug):
    ...
    related=[{"id":r.id,**r.to_dict()} for r in db.collection("articles")
             .where("category","==",art.get("category","")).where("status","==","published").limit(5).stream()
             if r.id!=doc.id][:4]
    comments=[]
...
def author_page(author_id):
    ...
    arts=[{"id":d.id,**d.to_dict()} for d in db.collection("articles")
          .where("author_id","==",author_id).where("status","==","published")
          .order_by("published_at",direction=firestore.Query.DESCENDING).stream()]
...
def sitemap():
    arts=[{"id":d.id,**d.to_dict()} for d in (db.collection("articles").where("status","==","published").stream() if db else [])]
    return Response(render_template("sitemap.xml",articles=arts,site_url=SITE_URL),mimetype="application/xml")

@app.route("/rss.xml")
def rss():
    arts=[{"id":d.id,**d.to_dict()} for d in (db.collection("articles").where("status","==","published")
          .order_by("published_at",direction=firestore.Query.DESCENDING).limit(20).stream() if db else [])]
    return Response(render_template("rss.xml",articles=arts,site_url=SITE_URL),mimetype="application/rss+xml")

...
app.secret_key = "TezKhabar@2026#SecretKey!XyZ789abc"
...
def admin_dashboard():
    ...
    total_views=sum(a.get("views",0) for a in [r for r in [{"id":d.id,**d.to_dict()} for d in recent_docs]])
...
def admin_chat_list():
    rooms = []
    if db:
        try: rooms = [{"id":d.id,**d.to_dict()} for d in db.collection("chat_rooms").where("active","==",True).order_by("created_at",direction=firestore.Query.DESCENDING).stream()]
        except: pass
...
def admin_chat_room(room_id):
    ...
    msgs = []
    try: msgs = [{"id":m.id,**{k:(v.isoformat() if hasattr(v,"isoformat") else v) for k,v in m.to_dict().items()}} for m in db.collection("chat_rooms").document(room_id).collection("messages").order_by("timestamp").limit(100).stream()]
    except: pass
...
def admin_chat_send(room_id):
    ...
    db.collection("chat_rooms").document(room_id).collection("messages").add({"text":text,"author":session.get("admin_name","Unknown"),"author_id":session.get("admin_id",""),"timestamp":datetime.utcnow(),"mentions":mentions[:10],"reply_to":reply_to})
    return jsonify({"ok":True})

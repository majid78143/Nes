...
# FIX: AD_SLOT_HEADER — triple quotes use kiye taaki HTML ke andar ke quotes se conflict na ho
...
# FIX: AD_SLOT_SIDEBAR — pehle URL tha, ab sahi slot ID hai
AD_SLOT_SIDEBAR      = "5369764793"
...
def admin_analytics():
    ...
    try:
        ...
        print(f"Analytics fetch error: {e}"); top=[]
    searches={}
    try:
...
def api_articles():
    ...
    try:
        if cat: docs=list(db.collection("articles").where("category","==",cat).where("status","==","published").order_by("published_at",direction=firestore.Query.DESCENDING).limit(lim).stream())
        else: docs=list(db.collection("articles").where("status","==","published").order_by("published_at",direction=firestore.Query.DESCENDING).limit(lim).stream())
    except Exception as e:
        print(f"api_articles fetch error: {e}"); docs=[]
...
def api_trending():
    ...
    try:
        docs=list(db.collection("articles").where("status","==","published").order_by("views",direction=firestore.Query.DESCENDING).limit(10).stream())
    except Exception as e:
        print(f"api_trending fetch error: {e}"); docs=[]
...
def api_search():
    ...
    try:
        docs=list(db.collection("articles").where("status","==","published").order_by("published_at",direction=firestore.Query.DESCENDING).limit(100).stream())
    except Exception as e:
        print(f"api_search fetch error: {e}"); docs=[]
...
def admin_chat_list():
    ...
    if db:
        try:
            rooms = [{"id":d.id,**d.to_dict()} for d in db.collection("chat_rooms").where("active","==",True).order_by("created_at",direction=firestore.Query.DESCENDING).stream()]
        except Exception as e:
...
def admin_chat_messages(room_id):
    ...
    try:
        msgs = [{"id":m.id,**{k:(v.isoformat() if hasattr(v,"isoformat") else v) for k,v in m.to_dict().items()}} for m in db.collection("chat_rooms").document(room_id).collection("messages").order_by("timestamp").limit(150).stream()]
    except Exception as e:
